...
trade_list = pd.DataFrame(pd.read_excel("K-stat 수출입 무역통계_2023.xlsx"))
...
data = pd.DataFrame(pd.read_excel('pca/total_container.xlsx'))
data_col = pd.DataFrame(pd.read_excel('pca/WEOOct2024all.xlsx'))
...
data = pd.DataFrame(pd.read_csv('top20_10years.csv'))
...
# Feature, target 분리
data = pd.DataFrame(pd.read_csv('container_13-23.csv')).iloc[::-1].reset_index(drop=True)
...
def regression(cell_id):
    weo_col = pd.DataFrame(pd.read_csv('weo.csv',encoding='cp949'))
